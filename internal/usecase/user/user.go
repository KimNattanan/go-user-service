...
func (u *UserUsecase) Update(ctx context.Context, id string, fields map[string]interface{}) error {
...
func (u *UserUsecase) LoginOrRegisterWithGoogle(ctx context.Context, userInfo map[string]interface{}) (*entity.User, error) {
	...
	fullName, _ := userInfo["name"].(string)
	...
	if user == nil {
		...
		if err := u.Update(ctx, user.ID, map[string]interface{}{
			"full_name":   fullName,
